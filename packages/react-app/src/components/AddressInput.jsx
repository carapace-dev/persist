...
export default function AddressInput(props) {
  ...
  const handleScannedAddress = (value) => {
    if (!!value) {
      if (ethers.utils.isAddress(value)){
        updateAddress(value);
      } else {
        if (value.includes('ethereum:')){
          const cleanAddress = value.split('ethereum:')[1].split('@')[0];
          if (ethers.utils.isAddress(cleanAddress)){
            updateAddress(cleanAddress);
          } else {
            updateAddress('Scanned address not valid: ', value);
          }
        } else {
          updateAddress('Scanned address not valid: ', value);
        }
